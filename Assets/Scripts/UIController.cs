...
public class UIController : MonoBehaviour
{
    ...
    {
        var time = Global.TimeFormat(GameManager.Instance.song.time);
        Instance.timePlayed.SetText($"{time[0]}:{time[1]}.{time[2]}");

        Instance.audioTime.SetText(GameManager.Instance.song.timeSamples.ToString());

        Instance.slider.value = GameManager.Instance.song.time / Global.song.length;
    }

    private void TriggerTime(float value)
    {
        var time = Global.TimeFormat(value * Global.song.length);
        Instance.timePlayed.SetText($"{time[0]}:{time[1]}.{time[2]}");
        
        GameManager.Instance.timePointer = value * Global.song.length;
    ...
    public static void InitAudioLabel()
    {
        Instance.audioTime.SetText("0");
        Instance.timePlayed.SetText("00:00.000");

        var time = Global.TimeFormat(Global.song.length);
